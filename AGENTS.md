...
- **Hero cube auto-cycles the top-10 most-voted games and links to `/cube`.** `HeroCube` (`components/HeroCube.tsx`) is a non-interactive rotating cube in the hero of `/`. Every `CYCLE_INTERVAL_MS` it advances to the next of the top-10 games by `vote_count`, expanding that game's votes via a `VoteCluster` (forward) while the previous game's votes collapse (reverse `VoteCluster` with `onExited` cleanup). The active game is reported up via an `onActiveGameChange` callback to `HeroCubeWithLabel`, the client wrapper that renders the bottom-right glitch label (`GlitchText`) — a per-character resolve-from-random animation. Cycle advancement uses the `setActiveIndex(prev => ...)` updater form (not a `let i` closure) to avoid the `react-hooks/set-state-in-effect` lint rule. Unselected game dots are dimmed to gray (`DIM_GRAY`); the active dot retains its game color. The three cube edges touching the (+,+,+) corner fade from acid to gray via drei `<Line vertexColors>`; all other edges are solid gray. In `app/page.tsx`, the `HeroCubeWithLabel` is wrapped in a Next.js `<Link href="/cube">` so clicking the hero cube navigates to the 3D cube page; the inner container keeps `pointer-events-none` so the canvas doesn't capture clicks (the Link does) and the cube is the visual affordance.
- **The `/cube` camera is driven by `CameraTarget` in `ThreeCube.tsx`.** `CameraTarget` owns the only `OrbitControls` (there is no standalone `<OrbitControls makeDefault />` anymore) and animates its `target` toward the selected game dot's position each frame (`TARGET_LERP_SPEED` lerp), back to origin on deselect. The camera translates by the same per-frame delta as the target so the cube's orientation stays fixed (only the rotation center moves) — do not move the target without also moving the camera, or the view will swing toward the new target. Selecting a dot also lerps the camera distance to `SELECTED_DISTANCE` (60% of the initial `sqrt(432)` ≈ 20.78); deselecting lerps it back to `INITIAL_DISTANCE`. The auto-zoom is cancelled by an OrbitControls `start` event listener (wheel/drag) so manual input is never fought — when adding zoom behavior, gate it behind a `resettingZoom` ref and clear that ref on `'start'`, never let it run unconditionally each frame. Constants (`INITIAL_TARGET`, `INITIAL_CAMERA`, `INITIAL_DISTANCE`, `SELECTED_DISTANCE`, `*_LERP_SPEED`) live at module scope in `ThreeCube.tsx`.
