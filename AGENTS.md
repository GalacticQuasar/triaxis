...
- **No global `scroll-behavior: smooth`.** Do NOT add `html { scroll-behavior: smooth }` or `data-scroll-behavior="smooth"` on `<html>`. A global smooth rule hijacks browser back/forward scroll restoration, so navigation animates from the top to the saved scroll position instead of instantly snapping to it. Smooth scroll is opt-in per-call (`VoteSliders`'s `window.scrollTo` inside a `requestAnimationFrame` after `setSaving(true)`).
...
- **`/discover` matches recompute live — no submit gate.** `DiscoverClient` derives `results` via `useMemo` from `target`/`weights`/`excludeIds`, so every pick toggle, slider drag, or weight change updates matches immediately. Do not reintroduce a `submitted`/`canSubmit` state or a "Find Similar"/"Recompute" button — that was removed deliberately. The right column's empty state is gated on `hasTarget` (Pick mode: `pickGames.length > 0`; Manual mode: always true).
- **`TargetVectorViz` is `/discover` Pick-mode only and its exit animation is load-bearing.** It renders above the Matches section (inside the right column, below the page header) only when `mode === 'pick'`. A `usePresence` hook inside the component keeps deselected game dots mounted with `animate-pop-out` for 200ms before unmounting (parallel enter/exit); the legend labels use `animate-fade-in`/`animate-fade-out` (no pop). The target avg marker slides via a ref-driven `transition: left` (450ms) — do not switch it to a re-render-driven position or it'll snap. Manual mode shows no `TargetVectorViz` (the target is direct slider input, no seed dots to visualize).
...
components/
  ...
  DiscoverClient.tsx      # Client taste-profile UI for /discover. Pick mode (select favorites → mean vector) / Manual mode (3 sliders → target vector). Per-axis emphasis weights (0-2×). Results recompute live via useMemo (no submit gate). localStorage persistence via lazy useState initializers. Includes EmphasisHelpModal. Pick/Manual toggle sits in the page header (top-right); ManualPanel has its own Reset (→ DEFAULT_MANUAL).
  TargetVectorViz.tsx     # Pick-mode-only component above the Matches section: per-axis track with a colored dot per selected seed game (positioned at its value) + a glowing target avg marker. Dots pop in/out via usePresence; labels fade. Avg marker slides via ref-driven transition.
